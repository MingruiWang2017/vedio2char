...
def ascii_art_convert(src_dir, dest_dir):
    ...
    for picture in picts_list:
        ...
        # 生成字符画图片
        create_ascii_picture(pixels, symbols, os.path.join(
            dest_dir, picture), x_size, y_size)
...
def start_convert(src_file):
    ...
    # 切割成图片
    slice_pic_cmd = 'ffmpeg.exe -i {0} -r 24 temp_pic/%06d.jpeg'.format(
        src_file)
    ...
    # 合成字符视频
    dst_name = os.path.join(os.path.dirname(src_file),
                            'ascii_' + os.path.basename(src_file))
    merge_ascii_video_cmd = 'ffmpeg -threads 2 -start_number 000001 -r 24 -i {0}/%06d.jpeg -i temp.mp3 -vcodec mpeg4 {1}'.format(
        'temp_ascii', dst_name)
